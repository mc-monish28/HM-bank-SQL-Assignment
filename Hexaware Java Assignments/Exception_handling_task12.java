...
interface ICustomerServiceProvider {
    ...
    float withdraw(long accNo, float amount) throws InvalidAccountException, InsufficientFundException, OverDraftLimitExceededException;
    boolean transfer(long fromAccNo, long toAccNo, float amount) throws InvalidAccountException, InsufficientFundException;
...
class BankServiceProviderImpl implements ICustomerServiceProvider, IBankServiceProvider {
    ...
    public Account findAccount(long accNo) {
        return accountList.stream().filter(acc -> acc.getAccountNumber() == accNo).findFirst().orElse(null);
    }
    ...
    public boolean transfer(long fromAccNo, long toAccNo, float amount) throws InvalidAccountException, InsufficientFundException {
        Account fromAccount = findAccount(fromAccNo);
        Account toAccount = findAccount(toAccNo);

        if (fromAccount == null) throw new InvalidAccountException("Sender account not found!");
        if (toAccount == null) throw new InvalidAccountException("Receiver account not found!");
        if (fromAccount.getBalance() < amount) throw new InsufficientFundException("Insufficient funds!");
    ...
    public static void main(String[] args) {
        while (true) {
            try {
                ...
                switch (choice) {
                    case 1 -> bankService.createAccount(new Customer("John", "john@example.com", "9999999999"), "Savings", 1000);
                    ...
                    case 7 -> System.out.println(Arrays.toString(bankService.listAccounts()));
